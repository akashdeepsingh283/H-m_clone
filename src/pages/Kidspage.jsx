...
const Kidspage = () => {
  ...
  const applyfilter=()=>{
    let productscopy=products.filter((item)=>item.category==='Kids').slice();
    if (search) {
  ...
  useEffect(()=>{
    ...
    useEffect(() => {
        setKidsdata(products.filter((item)=>item.category==='Kids'));
    }, [])

    const sortdata=()=>{
      if(Kidsdata.length>0){
      let fp=Kidsdata.slice();
      switch(sorttype){
        case 'low-high':
